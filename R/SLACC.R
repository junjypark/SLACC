SLACC = function(dat, mod = NULL, L = 5, batch = NULL, include_diag = TRUE, init = NULL,
                 maxIter = 30, eps = 1e-3, U_maxIter = 200, U_eps = 1e-3, 
                 lambda_U = NULL, tau = NULL, annealing = TRUE, lambda_warmup_iter = NULL){
  n = nrow(dat)
  p = ncol(dat)
  V = (sqrt(1 + 8*p) - 1) / 2
  ...
  else { nonzero = which(Ltrans(diag(V), d = TRUE) == 0) }
  p0 = length(nonzero)
  
  ## tuning
  if (is.null(tau)) { tau = 0.5 * sqrt(log(V*L) / n) }
  if (is.null(lambda_U)){ lambda_U = log(n) }
  
  if (annealing && is.null(lambda_warmup_iter)) {
  ...
  while (Iter < maxIter) {
    ...
    if (lambda_U > 0) {
      if (annealing) {
        if (Iter <= lambda_warmup_iter) { lambda_t = lambda_U * (Iter / lambda_warmup_iter) } 
        else { lambda_t = lambda_U }
      } else { lambda_t = lambda_U }
    } else { lambda_t = 0 }    
    ...
    crit = norm(U_prev - Uhat, type = "2") / sqrt(V * L)
    
    if (annealing) {
      if (Iter > lambda_warmup_iter && crit < eps) { break } 
      else { U_prev = U }
    } else {
      if (crit < eps) { break } 
      else { U_prev = U }
  ...
  ll = logLikSLACC_batch(
    dat[, nonzero, drop = FALSE], X, B[, active, drop = FALSE],
    S[nonzero, active, drop = FALSE], R[active, active, drop = FALSE],            
    sigma2_by_batch = sigma2_g[, active, drop = FALSE],
    phi2_by_batch = phi2_g,batch = batch )
  
  nparam = sum(U != 0) + sum(B != 0) + sum(sigma2_g != 0) + M
  
  estimates = list(
    A = A, S = S, U = U, B = B, R = R,sigma2 = sigma2_g, phi2 = phi2_g,
    resid_means = resid_mean_list
  )
  
  input = list(
    X = X, batch = batch, L = L,
    lambda_U = lambda_U, tau = tau, maxIter = maxIter,
    eps = eps, U_maxIter = U_maxIter, U_eps = U_eps,
    include_diag = include_diag
  )
  
  measure = list(logLik = ll, df = nparam, Iter = Iter)
