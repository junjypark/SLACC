bilinear_als3 <- function(Y, A, w,
  ...
  # Q: NULL / L×L / list of L×L (batch별 Q_i), bilinear_admm3와 동일한 의미
  # U0,V0: 초기값 (없으면 HOSVD 기반 초기화와 동일하게 eigen로)
  # include_diag: FALSE면 bilinear_admm3와 같이 diag(Y_ij)를 fitted diag로 덮어서
  #               off-diagonal 기준으로만 loss를 맞추는 트릭 사용
  # gamma: 수치 안정성을 위한 작은 ridge (K에 gamma * I 추가)
  ...
  for (it in seq_len(maxit)) {
    ...
    for (i in seq_len(n)) {
      ...
      if (!include_diag) {
        # off-diagonal만 맞추고 싶을 때:
        # diag(Yi)를 현재 (U, V, a_ij)로부터의 fitted diag로 덮어씀
        a        <- diag(Ai)           # a_ij (L×1)
  ...
  ## 최종에서는 U ≈ V. 그냥 U를 리턴하거나 (U+V)/2로 symmetrize해도 됨.
  U_final <- (U + V) / 2
